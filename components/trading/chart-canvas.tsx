...
import { loadYearlyDataset, type LoadedYearlyDataset, type YearDatasetId } from '@/lib/core/yearly-data-loader';
import {
  Maximize2,
  Minimize2,
  ZoomIn,
...
export const ChartCanvas: React.FC<ChartCanvasProps> = ({
  ...
  // تاریخچهٔ پژوهشی انتخاب‌شده فقط هنگام انتخاب تایم‌فریم بار می‌شود.
  useEffect(() => {
  ...
  const lastCandle = activePrimaryCandles[activePrimaryCandles.length - 1] || candles[candles.length - 1];
    ...
    >
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between mb-2.5 px-1 text-xs shrink-0 gap-2 border-b border-[var(--border-subtle)] pb-2">
        <div className="flex items-center justify-between md:justify-start gap-2 font-mono flex-wrap">
          ...
          <div className="flex items-center bg-[var(--bg-canvas)] rounded-xl p-0.5 border border-[var(--border-subtle)] text-[11px] font-mono" aria-label="سال دادهٔ تاریخی">
            {(['2024', '2025'] as YearDatasetId[]).map((year) => (
              <button
                key={year}
                type="button"
      ...
      {!isSplitView ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-3 w-full flex-1 min-h-0">
          {/* چارت اول: تایم‌فریم اولیه انتخابی */}
          <SingleChartPane
            symbol={symbol}
            candles={activePrimaryCandles}
            timeframe={primaryTimeframe}
          ...
          <SingleChartPane
            symbol={symbol}
            candles={activeSecondaryCandles}
            timeframe={secondaryTimeframe}
            title={secondaryTimeframe === 'D1' ? `چارت ۲: ساختار روزانهٔ کامل ${selectedYear}` : `چارت ۲: ساختار روند کلان (${secondaryTimeframe}) — ${selectedYear}`}
            activeCandidate={activeCandidate}
            multiTimeframeLevels={multiTimeframeLevels}
            monteCarloCone={[]}
            crosshairPrice={activeCrosshairPrice}
